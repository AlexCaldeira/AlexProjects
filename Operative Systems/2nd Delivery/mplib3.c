...
pthread_mutex_t    *single_mutex_array;
pthread_cond_t     *wait_for_free_space_array;
pthread_cond_t     *wait_for_messages_array;
...
int inicializarMPlib(int capacidade_de_cada_canal, int ntasks) {
  ...
  single_mutex_array = (pthread_mutex_t*)malloc(sizeof(pthread_mutex_t)*ntasks*ntasks);
  wait_for_free_space_array = (pthread_cond_t*)malloc(sizeof(pthread_cond_t)*ntasks*ntasks);
  wait_for_messages_array = (pthread_cond_t*)malloc(sizeof(pthread_cond_t)*ntasks*ntasks);
  ...
  for (i=0; i<ntasks; i++) {
    for (j=0; j<ntasks; j++) {
      ...
      if(pthread_mutex_init(&single_mutex_array[i*number_of_tasks+j], NULL) != 0) {
		fprintf(stderr, "\nErro ao inicializar mutex\n");
		return -1;     
			}
			
	  if(pthread_cond_init(&wait_for_messages_array[i*number_of_tasks+j], NULL) != 0){
		fprintf(stderr, "\nErro ao inicializar variável de condição\n");
		return -1;
			} 
			
	  if(pthread_cond_init(&wait_for_free_space_array[i*number_of_tasks+j], NULL) != 0){
		fprintf(stderr, "\nErro ao inicializar variável de condição\n");
		return -1;
			} 
		}
...
void libertarMPlib() {
    ...
    for (j=0; j<number_of_tasks; j++) {
      ...
      while (mess) {
        ...
        mess = (Message_t*) leQueRemFirst(channel->message_list);
		  ...
		  if(pthread_mutex_destroy(&single_mutex_array[i*number_of_tasks+j]) != 0) {
			 fprintf(stderr, "\nErro ao destruir mutex\n");
			 exit(EXIT_FAILURE);
				}
  
		  if(pthread_cond_destroy(&wait_for_free_space_array[i*number_of_tasks+j]) != 0) {
			 fprintf(stderr, "\nErro ao destruir variável de condição\n");
			 exit(EXIT_FAILURE);
				}

		  if(pthread_cond_destroy(&wait_for_messages_array[i*number_of_tasks+j]) != 0) {
			 fprintf(stderr, "\nErro ao destruir variável de condição\n");
			 exit(EXIT_FAILURE);
			}
  ...
  free (channel_array);
  free(wait_for_free_space_array);
  free(wait_for_messages_array);
}
...
int receberMensagem(int tarefaOrig, int tarefaDest, void *buffer, int tamanho) {
  ...
  if(pthread_mutex_lock(&single_mutex_array[tarefaDest*number_of_tasks+tarefaOrig]) != 0) {
  ...
  while (!mess) {
    if(pthread_cond_wait(&wait_for_messages_array[tarefaDest*number_of_tasks+tarefaOrig], &single_mutex_array[tarefaDest*number_of_tasks+tarefaOrig]) != 0) {
  ...
  if(pthread_cond_broadcast(&wait_for_free_space_array[tarefaDest*number_of_tasks+tarefaOrig]) != 0) {
    fprintf(stderr, "\nErro ao desbloquear variável de condição\n");
    return -1;
  }

  if(pthread_mutex_unlock(&single_mutex_array[tarefaDest*number_of_tasks+tarefaOrig]) != 0) {
...
int enviarMensagem(int tarefaOrig, int tarefaDest, void *msg, int tamanho) {
  ...
  if (channel_capacity >0) {
    while (leQueSize(channel->message_list) >= channel_capacity) {
      if(pthread_cond_wait(&wait_for_free_space_array[tarefaDest*number_of_tasks+tarefaOrig], &single_mutex_array[tarefaDest*number_of_tasks+tarefaOrig]) != 0) {
        fprintf(stderr, "\nErro ao esperar pela variável de condição\n");
        return -1;
      }
    }
  ...
  pthread_cond_broadcast(&wait_for_messages_array[tarefaDest*number_of_tasks+tarefaOrig]);
